...
fn testSimpleGet() {
    ...
    let response = httpGet("https://api.github.com/zen");
...
fn testGetJson() {
    ...
    let response = httpGetJson("https://api.github.com/users/github");
...
fn testPostJson() {
    ...
    let response = httpPostJson("https://jsonplaceholder.typicode.com/posts", jsonData);
...
fn testSimpleDownload() {
    ...
    let content = downloadString("https://httpbin.org/get");
    Std.IO.println("Downloaded content (first 200 chars):");
    Std.IO.println(substring(content, 0, 200));
    Std.IO.println("...");
    Std.IO.println("");
}
...
fn testUrlUtilities() {
    ...
    Std.IO.println(urlDecode(encoded));
    Std.IO.println("");
}
...
fn testNetworkUtilities() {
    ...
    Std.IO.println(getHostname());
    Std.IO.print("Local IP: ");
    Std.IO.println(getLocalIP());
    Std.IO.println("");
...
fn testStatusCodes() {
    ...
    let mut k = 0;
    while (k < length(codes)) {
        let code = codes[k];
        let text = getStatusText(code);
        
        Std.IO.print(toString(code));
        Std.IO.print(" ");
        Std.IO.print(text);
        Std.IO.print(" - ");
...
fn testWebScraper() {
    ...
    Std.IO.print(toString(length(html)));
    Std.IO.println(" bytes");
    
    // Extract title (simple string search)
    let titleStart = indexOf(html, "<title>");
    let titleEnd = indexOf(html, "</title>");
    
    if (titleStart != -1) {
        if (titleEnd != -1) {
            let title = substring(html, titleStart + 7, titleEnd);
...
fn testRateLimitedRequests() {
    ...
    while (n <= 5) {
        ...
        Std.IO.print(toString(n));
        Std.IO.println("...");
        
        let response = httpGet("https://httpbin.org/uuid");
        
        if (length(response) > 0) {
            Std.IO.print("  Response received: ");
            Std.IO.print(substring(response, 0, 50));
            Std.IO.println("...");
        }
...
fn testHttpMethods() {
    ...
    let getResp = httpGet("https://httpbin.org/get");
    if (length(getResp) > 0) {
    ...
    let postResp = httpPost("https://httpbin.org/post", "test data");
    if (length(postResp) > 0) {
        Std.IO.println("  ✓ POST successful");
    }
    
    // PUT
    Std.IO.println("Testing PUT...");
    let putResp = httpPut("https://httpbin.org/put", "update data");
    if (length(putResp) > 0) {
    ...
    let delResp = httpDelete("https://httpbin.org/delete");
    if (length(delResp) > 0) {
...
fn testErrorHandling() {
    ...
    let response = httpGet("https://this-domain-definitely-does-not-exist-12345.com");
...
fn testFileDownload() {
    ...
    let content = downloadString("https://raw.githubusercontent.com/github/gitignore/main/LICENSE");
...
fn testBasicAuth() {
    ...
    let response = httpGetWithBasicAuth(
...
fn runNetworkTestSuite() {
    ...
    Std.IO.print(toString(duration));
