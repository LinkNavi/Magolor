...
public:
    ...
    const std::unordered_map<std::string, JsonValue>& asObject() const { return objVal_; }
...
private:
    ...
    void serializeTo(std::ostream& os) const {
        switch (type_) {
            ...
            case Bool: os << (boolVal_ ? "true" : "false"); break;
            case Int: os << intVal_; break;
            case Float: os << floatVal_; break;
            case String: 
                os << '"';
                for (char c : strVal_) {
                    if (c == '"') os << "\\\"";
...
private:
    ...
    static JsonValue parseBool(const std::string& s, size_t& pos) {
        if (s[pos] == 't') { pos += 4; return JsonValue(true); }
