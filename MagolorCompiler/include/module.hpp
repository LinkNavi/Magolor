...
struct ModuleInfo {
  std::string name;
  std::string filepath;
  std::unordered_map<std::string, ClassDecl> publicClasses;
};

class ModuleResolver {
public:
  // Load a module by name
  ModuleInfo loadModule(const std::string &moduleName);

  // Check if a symbol is accessible
  bool isPublic(const std::string &className, const std::string &memberName);

  // Resolve a using declaration
  void resolveUsing(const UsingDecl &usingDecl);

  // Get all visible symbols in current scope
  std::vector<std::string> getVisibleSymbols();
...
private:
  ...
  // Find module file
  std::string findModuleFile(const std::string &name);
};
...
public:
  ...
  void registerModule(ModulePtr module) {
    ...
    if (existing != modules.end() &&
        existing->second->packageName == module->packageName) {
  ...
  Symbol *resolveSymbol(const std::string &modulePath,
  ...
  // Get all modules
  const std::map<std::string, ModulePtr> &getModules() const { return modules; }
...
private:
  ...
  ModuleRegistry() { registerStdModules(); }
};
...
public:
  ...
  static std::string filePathToModuleName(const std::string &filepath,
    ...
    if (!packageName.empty() &&
      ...
      if (lastSlash != std::string::npos) {
        ...
        if (filename.size() > 3 &&
            filename.substr(filename.size() - 3) == ".mg") {
  ...
  resolveImportPath(const std::string &currentModule,
...
public:
  ...
  ResolveResult resolve(ModulePtr module) {
    ...
    for (const auto &usingDecl : module->ast.usings) {
      ...
      // Try to resolve
      auto candidates =
          ModuleResolver::resolveImportPath(module->name, usingDecl.path);
      ...
      if (!found) {
        ...
        result.error = "Cannot find module '" + importPath + "' imported in " +
                       module->name;
        return result;
      }
...
public:
  ...
  bool lookup(const std::string &name, std::string *outType = nullptr,
              void **outDecl = nullptr) {
    auto it = symbols.find(name);
    if (it != symbols.end()) {
      if (outType)
        *outType = it->second.type;
      if (outDecl)
        *outDecl = it->second.declaration;
      return true;
    }
  ...
  ResolveResult resolve(ModulePtr module) {
    ...
    for (const auto &importName : module->imports) {
      ...
      if (!imported) {
        ...
        result.errors.push_back("DEBUG: Could not find imported module: " +
                                importName);
        continue;
      }
      ...
      // Extract the last part of the module name (e.g., "testlib" from
      // "testlib")
    ...
    std::visit(
        [&](auto &&s) {
          ...
          if constexpr (std::is_same_v<T, LetStmt>) {
            ...
            if (s.value)
              resolveExpr(s.value, scope, result);
        ...
        [&](auto &&e) {
          ...
          if constexpr (std::is_same_v<T, IdentExpr>) {
            ...
            for (const auto &st : e.body)
              resolveStmt(st, lambdaScope.get(), result);
