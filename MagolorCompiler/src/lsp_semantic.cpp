            ...
            if (sym) {
                ...
                if (parenStart != std::string::npos && parenEnd != std::string::npos) {
                    ...
                    if (arrowPos != std::string::npos) {
                        ...
                        std::string retType = line.substr(arrowPos + 2, bracePos - arrowPos - 2);
...
void SemanticAnalyzer::parseImport(const std::string& line, Scope* scope) {
    ...
    if (importPath == "Std.IO") {
        ...
        import.importedSymbols = {
            "create", "insert", "get", "getOr", "contains",
            "remove", "size", "isEmpty", "clear", "keys", "values"
        };
    } else if (importPath == "Std.Set") {
        import.importedSymbols = {
            "create", "insert", "contains", "remove", "size",
            "isEmpty", "clear", "toArray", "union_", "intersection", "difference"
        };
    } else if (importPath == "Std.File") {
        import.importedSymbols = {
            "exists", "isFile", "isDirectory", "createDir", "remove",
            "removeAll", "copy", "rename", "size"
        };
