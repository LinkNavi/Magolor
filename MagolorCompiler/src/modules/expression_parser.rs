...
impl<'a> ExpressionParser<'a> {
    ...
    // Ternary operator: condition ? then_val : else_val
    ...
    fn parse_primary(&mut self) -> Result<ASTValue, String> {
        match self.peek() {
            ...
            Some(Token::Integer64(n)) => {
                let val = ASTValue::Int64(*n);
                self.advance();
                Ok(val)
            }
            Some(Token::Float32(f)) => {
                let val = ASTValue::Float32(*f);
                self.advance();
                Ok(val)
            }
            Some(Token::Float64(f)) => {
                let val = ASTValue::Float64(*f);
                self.advance();
                Ok(val)
            }
            Some(Token::DefaultFloat(f)) => {
                let val = ASTValue::Float32(*f);
                self.advance();
                Ok(val)
            }
