...
impl IRBuilder {
    ...
    fn build_function(&mut self, func: FunctionDef) -> Result<(), String> {
        ...
        if !self.has_terminator(&ir_func) {
            if ir_func.return_type == IRType::Void {
                ...
                let default_val = IRValue::Constant(IRConstant::I32(0));
        ...
        match stmt {
            Statement::VarDecl { name, var_type, value, is_mutable: _ } => {
                ...
                if let Some(val) = value {
                    ...
                    let zero = IRValue::Constant(IRConstant::I32(0));
                    self.emit_instruction(func, IRInstruction::Store {
                        addr: addr.clone(),
                        value: zero,
                        ty: ty.clone(),
            ...
            Statement::CompoundAssignment { target, op, value } => {
                ...
                let instr = match op {
                    ...
                    _ => return Err("Unsupported compound assignment".to_string()),
                };
            ...
            Statement::ForEach { item, item_type, collection, body } => {
                self.build_foreach_loop(func, item, item_type, collection, body)
            }
    ...
    fn build_foreach_loop(
        ...
        // Simple foreach: iterate through array elements
        // For now, we'll implement a simple version that works with stack arrays
        
        let collection_reg = self.build_expression(func, collection)?;
        let ty = self.convert_type(&item_type);
        
        // Create loop variable
        let item_local = self.local_counter;
        self.local_counter += 1;
        let item_addr = IRValue::Local(item_local);
        
        // Create index variable
        let index_local = self.local_counter;
        self.local_counter += 1;
        let index_addr = IRValue::Local(index_local);
        
        // Initialize index to 0
        self.emit_instruction(func, IRInstruction::Store {
            addr: index_addr.clone(),
            value: IRValue::Constant(IRConstant::I32(0)),
            ty: IRType::I32,
        });
        
        // For simplicity, assume array length is known (5 in your example)
        // In a real implementation, this would be stored with the array
        let array_length = 5;
        ...
        let index_reg = self.next_register();
        self.emit_instruction(func, IRInstruction::Load {
            dst: index_reg,
            addr: index_addr.clone(),
            ty: IRType::I32,
        ...
        self.emit_instruction(func, IRInstruction::Cmp {
            dst: cmp_reg,
            op: CmpOp::Lt,
            lhs: IRValue::Register(index_reg),
            rhs: IRValue::Constant(IRConstant::I32(array_length)),
            ty: IRType::I32,
        });
        ...
        // Body: load array[index] into item
        self.switch_to_block(func, body_block);
        
        // Calculate array element address (simplified)
        let element_reg = self.next_register();
        self.emit_instruction(func, IRInstruction::Add {
            dst: element_reg,
            lhs: collection_reg.clone(),
            rhs: IRValue::Register(index_reg),
            ty: IRType::I32,
        });
        
        // Store element in item variable
        self.emit_instruction(func, IRInstruction::Store {
            addr: item_addr.clone(),
            value: IRValue::Register(element_reg),
            ty: ty.clone(),
        });
        
        // Register item in symbol table
        self.push_scope();
        self.symbol_table.last_mut().unwrap().insert(item.clone(), (item_addr, ty));
        ...
        let current_index_reg = self.next_register();
        self.emit_instruction(func, IRInstruction::Load {
            dst: current_index_reg,
            addr: index_addr.clone(),
            ty: IRType::I32,
        });
        
        let new_index_reg = self.next_register();
        self.emit_instruction(func, IRInstruction::Add {
            dst: new_index_reg,
            lhs: IRValue::Register(current_index_reg),
        ...
        self.emit_instruction(func, IRInstruction::Store {
            addr: index_addr,
            value: IRValue::Register(new_index_reg),
            ty: IRType::I32,
        });
    ...
    fn build_expression(&mut self, func: &mut IRFunction, expr: ASTValue) -> Result<IRValue, String> {
        match expr {
            ...
            ASTValue::ArrayLiteral(elements) => {
                // Allocate space for array
                let array_local = self.local_counter;
                self.local_counter += elements.len();
                
                // Store each element
                for (i, elem) in elements.iter().enumerate() {
                    let elem_val = self.build_expression(func, elem.clone())?;
                    let elem_addr = IRValue::Local(array_local + i);
                    self.emit_instruction(func, IRInstruction::Store {
                        addr: elem_addr,
                        value: elem_val,
                        ty: IRType::I32,
            ...
            _ => {
                Ok(IRValue::Constant(IRConstant::I32(0)))
            }
        }
    }
