...
void StdLibLoader::init(const std::string& path) {
    ...
    for (const auto& searchPath : searchPaths) {
        if (fs::exists(searchPath) && fs::is_directory(searchPath)) {
            ...
            std::cerr << "[StdLibLoader] Found stdlib at: " << searchPath << std::endl;
            break;
        }
    }
    
    if (stdlibPath.empty()) {
        std::cerr << "[StdLibLoader] WARNING: stdlib directory not found!" << std::endl;
        initialized = true;
        return;
    }
    ...
    std::cerr << "[StdLibLoader] Loaded " << modules.size() << " modules" << std::endl;
}
...
void StdLibLoader::discoverModules() {
    ...
    for (const auto& entry : fs::recursive_directory_iterator(stdlibPath)) {
        ...
        if (entry.path().extension() != ".mg") continue;
        ...
        std::cerr << "[StdLibLoader] Discovered module: " << moduleName 
                  << " from " << relPath << std::endl;
    }
}
...
bool StdLibLoader::hasModule(const std::string& modulePath) const {
    ...
    for (const auto& [path, _] : modules) {
        ...
        std::transform(lowerPath.begin(), lowerPath.end(), lowerPath.begin(), ::tolower);
        std::transform(lowerSearch.begin(), lowerSearch.end(), lowerSearch.begin(), ::tolower);
        if (lowerPath == lowerSearch) return true;
    }
    return false;
}
...
void StdLibLoader::parseModuleFile(StdModule& module) {
    ...
    if (!file) {
        std::cerr << "[StdLibLoader] Failed to open: " << module.filePath << std::endl;
        return;
    }
    ...
    std::cerr << "[StdLibLoader] Parsed " << module.fullPath 
              ...
              << ", " << module.metadata.linkFlags.size() << " link flags" << std::endl;
}
...
void StdLibLoader::extractFunctions(const std::string& source, StdModule& module) {
    ...
    for (auto it = begin; it != end; ++it) {
        ...
        func.isStatic = source.substr((*it).position(), (*it).length()).find("static") != std::string::npos;
...
void StdLibLoader::extractClasses(const std::string& source, StdModule& module) {
    ...
    for (auto it = begin; it != end; ++it) {
        ...
        std::regex constRegex(R"(pub\s+static\s+([a-zA-Z_][a-zA-Z0-9_]*)\s*:\s*([a-zA-Z_][a-zA-Z0-9_<>]*)\s*=)");
        auto cBegin = std::sregex_iterator(classBody.begin(), classBody.end(), constRegex);
        ...
        for (auto mit = mBegin; mit != mEnd; ++mit) {
            ...
            method.returnType.erase(0, method.returnType.find_first_not_of(" \t\n\r"));
...
void StdLibLoader::extractConstants(const std::string& source, StdModule& module) {
...
std::vector<std::pair<std::string, std::string>> StdLibLoader::parseParams(const std::string& paramStr) {
...
std::string StdLibLoader::extractDocComment(const std::string& source, size_t pos) {
...
std::string StdLibCodeGen::extractCppCode(const std::string& source, const std::string& funcName) {
    // Find the function
    std::regex funcStartRegex("pub\\s+(?:static\\s+)?fn\\s+" + funcName + "\\s*\\(");
    std::smatch match;
...
void StdLibLoader::extractMetadata(const std::string& source, StdModule& module) {
    // Extract @link { ... }
    ...
    while (std::regex_search(searchStart, source.cend(), match, linkRegex)) {
        ...
        while (iss >> flag) {
            ...
            if (start != std::string::npos && end != std::string::npos) {
                module.metadata.linkFlags.push_back(flag.substr(start, end - start + 1));
            }
        }
    ...
    while (std::regex_search(searchStart, source.cend(), match, includeRegex)) {
        ...
        auto tokensBegin = std::sregex_iterator(includes.begin(), includes.end(), tokenRegex);
    ...
    std::regex cimportRegex(R"(@?cimport\s*(?:\{\s*([^}]+)\}|\"([^\"]+)\"|<([^>]+)>))");
    searchStart = source.cbegin();
    
    while (std::regex_search(searchStart, source.cend(), match, cimportRegex)) {
        std::string header;
        if (match[1].matched) header = match[1].str();
        else if (match[2].matched) header = match[2].str();
        else if (match[3].matched) header = match[3].str();
...
std::vector<std::string> StdLibLoader::getLinkFlags(const std::string& modulePath) {
    return getModuleMetadata(modulePath).linkFlags;
}

std::vector<std::string> StdLibLoader::getCppIncludes(const std::string& modulePath) {
    return getModuleMetadata(modulePath).cppIncludes;
}

std::vector<std::string> StdLibLoader::collectLinkFlags(const std::unordered_set<std::string>& usedModules) {
...
std::vector<std::string> StdLibLoader::collectIncludes(const std::unordered_set<std::string>& usedModules) {
...
std::string CHeaderParser::resolveHeaderPath(const std::string& headerPath) {
    ...
    if (!cleanPath.empty() && (cleanPath.back() == '>' || cleanPath.back() == '"')) {
...
CHeader* CHeaderParser::parseHeader(const std::string& headerPath) {
    ...
    if (absPath.empty()) {
        std::cerr << "[CHeaderParser] Could not resolve: " << headerPath << std::endl;
        return nullptr;
    }
...
void CHeaderParser::parseHeaderContent(const std::string& content, CHeader& header) {
...
void CHeaderParser::extractFunctions(const std::string& content, CHeader& header) {
...
void CHeaderParser::extractStructs(const std::string& content, CHeader& header) {
    // Match struct definitions
    std::regex structRegex(
        R"((?:typedef\s+)?struct\s+([a-zA-Z_][a-zA-Z0-9_]*)\s*\{([^}]*)\})"
    );
    
    auto begin = std::sregex_iterator(content.begin(), content.end(), structRegex);
    auto end = std::sregex_iterator();
...
void CHeaderParser::extractTypedefs(const std::string& content, CHeader& header) {
    // Match simple typedefs (not struct/enum)
    std::regex typedefRegex(
        R"(typedef\s+([a-zA-Z_][a-zA-Z0-9_*\s]+)\s+([a-zA-Z_][a-zA-Z0-9_]*)\s*;)"
    );
    
    auto begin = std::sregex_iterator(content.begin(), content.end(), typedefRegex);
    auto end = std::sregex_iterator();
...
std::vector<CSymbol> CHeaderParser::getFunctions(const std::string& headerPath) {
...
CSymbol* CHeaderParser::findSymbol(const std::string& headerPath, const std::string& name) {
    auto* header = parseHeader(headerPath);
