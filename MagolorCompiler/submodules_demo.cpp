...
void testSecurity() {
    ...
    Std::print(((((std::string("  ✓ 16-char token: ") + token16) + std::string(" (len: ")) + Std::String::length(token16)) + std::string(")\n")));
    Std::print(((((std::string("  ✓ 32-char token: ") + token32) + std::string(" (len: ")) + Std::String::length(token32)) + std::string(")\n")));
    Std::print(((((std::string("  ✓ 64-char token: ") + token64) + std::string(" (len: ")) + Std::String::length(token64)) + std::string(")\n")));
    Std::print(std::string("\nCSRF Protection:\n"));
    auto csrf = Std::Network::Security::generateCsrfToken();
    Std::print(((std::string("  ✓ CSRF token: ") + csrf) + std::string("\n")));
    Std::print(((std::string("  ✓ Length: ") + Std::String::length(csrf)) + std::string(" chars\n")));
    Std::print(std::string("\nRate Limiting:\n"));
    ...
    while ((count < 7)) {
        ...
        if (allowed) {
            Std::print(((std::string("  ✓ Request ") + count) + std::string(" allowed\n")));
        }
        else {
            Std::print(((std::string("  ✗ Request ") + count) + std::string(" blocked (rate limit reached)\n")));
        }
        count = (count + 1);
    }
}
