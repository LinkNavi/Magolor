...
impl Parser {
    ...
    fn parse_top_level(&mut self) -> Result<Option<TopLevel>, String> {
        ...
        match self.peek() {
            ...
            | Some(Token::BoolType)
            | Some(Token::CharType)
            | Some(Token::Ident(_)) => self.parse_function(access, is_static),
    ...
    fn parse_function(
        ...
        self.expect(Token::RParen)?;
        let body = self.parse_block()?;
        Ok(Some(TopLevel::Function(FnDef {
            name,
            params,
            ret,
            body,
    ...
    fn parse_primary(&mut self) -> Result<Expr, String> {
        match self.peek() {
            ...
            Some(Token::This) | Some(Token::SelfKw) => {
                self.advance();
                Ok(Expr::This)
            }
            Some(Token::New) => {
